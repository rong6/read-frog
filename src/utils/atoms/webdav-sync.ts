/**
 * Jotai atoms driving the WebDAV conflict-resolution dialog.
 *
 * Mirrors `src/utils/atoms/google-drive-sync.ts` exactly — the only difference is
 * that the merge helpers come from `@/utils/webdav/conflict-merge`. The two
 * backends keep separate atom instances so an in-flight Drive resolution and an
 * in-flight WebDAV resolution can never read each other's state.
 */

import type { Config } from "@/types/config/config"
import type { ApplyResolutionsResult, DiffConflictsResult } from "@/utils/webdav/conflict-merge"
import { atom } from "jotai"
import { applyResolutions, detectConflicts } from "@/utils/webdav/conflict-merge"
