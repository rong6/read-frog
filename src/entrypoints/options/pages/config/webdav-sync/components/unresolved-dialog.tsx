/**
 * Conflict-resolution dialog for WebDAV sync: shown when both the local config
 * and the config on the server changed since the last sync. Structurally
 * identical to the Google Drive `unresolved-dialog.tsx`; the sync identity is the
 * WebDAV account (`username@server/directory`) instead of a Google email.
 */
...
import { getWebDAVAccountId, syncMergedConfig } from "@/utils/webdav/sync"
...
function DialogContent({ onResolved, onCancelled }: DialogContentProps) {
  ...
  const account = useMemo(
    () => (isWebDAVConfigured(webdav) ? getWebDAVAccountId(webdav) : null),
    [webdav],
  )
  ...
  const handleConfirm = async () => {
    ...
    if (!account) {
      toastManager.add({
        type: "error",
        title: i18n.t("options.config.sync.webdav.notConfigured"),
      })
      return
    }
    setIsConfirming(true)
    try {
      await syncMergedConfig(resolvedConfigResult.config, account)
      onResolved()
    } catch (error) {
    ...
    <AlertDialogContent className="flex max-h-[90vh] flex-col overflow-hidden data-[size=default]:max-w-[calc(100vw-2rem)] data-[size=default]:md:max-w-2xl data-[size=default]:lg:max-w-4xl data-[size=default]:xl:max-w-5xl">
      <AlertDialogHeader>
        <AlertDialogTitle className="flex items-center gap-2">
          ...
          {i18n.t("options.config.sync.webdav.unresolved.title")}
        </AlertDialogTitle>
        <AlertDialogDescription>
          {i18n.t("options.config.sync.webdav.unresolved.description")}
        </AlertDialogDescription>
      </AlertDialogHeader>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {status.allResolved ? (
            !status.isValid ? (
              <span className="flex items-center gap-1 text-red-600 dark:text-red-400">
                ...
                {i18n.t("options.config.sync.webdav.unresolved.configInvalid")}
              </span>
            ) : (
              <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
                <Icon icon="tabler:circle-check-filled" className="size-4" />
                {i18n.t("options.config.sync.webdav.unresolved.configValid")}
              </span>
            )
          ) : (
            <span className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
              <Icon icon="tabler:circle-dashed-check" className="size-4" />
              {i18n.t("options.config.sync.webdav.unresolved.resolveToContinue")}
            </span>
          )}
        </div>
        <div className="flex gap-2">
          ...
          >
            ...
            {i18n.t("options.config.sync.webdav.unresolved.useAllLocal")}
          </Button>
          <Button
            size="sm"
            variant="outline"
          ...
          >
            ...
            {i18n.t("options.config.sync.webdav.unresolved.useAllRemote")}
          </Button>
        </div>
      </div>
      ...
      {status.validationError && (
        <Alert variant="destructive">
          <Icon icon="tabler:alert-circle-filled" className="size-4" />
          <AlertTitle>
            {i18n.t("options.config.sync.webdav.unresolved.validationAlert.title")}
          </AlertTitle>
          <AlertDescription>
            <p>{i18n.t("options.config.sync.webdav.unresolved.validationAlert.description")}</p>
            <ul className="list-inside list-disc text-xs">
              ...
              {status.validationError.issues.length > 5 && (
                <li>
                  {i18n.t("options.config.sync.webdav.unresolved.moreErrors", [
                    status.validationError.issues.length - 5,
                  ])}
                </li>
              )}
      ...
      <AlertDialogFooter>
        <AlertDialogCancel disabled={isConfirming} onClick={handleCancel}>
          {i18n.t("options.config.sync.webdav.unresolved.cancel")}
        </AlertDialogCancel>
        <AlertDialogAction
          disabled={!canConfirm}
          onClick={(e) => {
        ...
        >
          {isConfirming
            ...
            : i18n.t("options.config.sync.webdav.unresolved.confirm")}
        </AlertDialogAction>
      </AlertDialogFooter>
    </AlertDialogContent>
  )
      ...
      <div className="flex items-center gap-4 border-b bg-muted px-4 py-2 text-xs">
        {status.conflictCount > 0 && (
          <span className="text-zinc-700 dark:text-zinc-300">
            {i18n.t("options.config.sync.webdav.unresolved.progress", [
        ...
        <div className="ml-auto flex items-center gap-4 text-zinc-600 dark:text-zinc-400">
          <div className="flex items-center gap-2">
            ...
            <span>{i18n.t("options.config.sync.webdav.unresolved.localValue")}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-blue-500" />
            <span>{i18n.t("options.config.sync.webdav.unresolved.remoteValue")}</span>
          </div>
        </div>
      </div>
