/**
 * A single conflicting config field rendered inside the WebDAV conflict tree:
 * the "what changed" header plus the two selectable local/remote rows.
 * Mirrors the Google Drive `unresolved-field.tsx`.
 */
...
import { useWebDAVConflictField } from "@/hooks/use-webdav-unresolved-field"
...
export function ConflictField({ pathKey, indent }: ConflictFieldProps) {
  const { conflict, resolution, selectLocal, selectRemote, reset } = useWebDAVConflictField(pathKey)
  ...
  const getIconAndLabel = () => {
    const label = bothChanged
      ? i18n.t("options.config.sync.webdav.unresolved.bothChanged")
      : localChanged
        ? i18n.t("options.config.sync.webdav.unresolved.localChanged")
        : i18n.t("options.config.sync.webdav.unresolved.remoteChanged")
      ...
      <div className="flex h-8 items-center py-1 ps-(--indent)">
        ...
        {resolution && (
          ...
          >
            ...
            {i18n.t("options.config.sync.webdav.unresolved.reset")}
          </Button>
        )}
      </div>
