/**
 * Collapsible JSON tree of the merged config shown in the WebDAV conflict dialog.
 * Conflicting paths are replaced by an interactive `ConflictField`. Mirrors the
 * Google Drive `json-tree-view.tsx`, bound to the WebDAV conflict atoms.
 */
...
import { diffConflictsResultAtom } from "@/utils/atoms/webdav-sync"
...
export function JsonTreeView({ resolvedConfig }: { resolvedConfig: Config }) {
  ...
  const formatFolderLabel = (value: unknown, childrenCount: number): string => {
    const countText =
      childrenCount === 1
        ? `${childrenCount} ${i18n.t("options.config.sync.webdav.unresolved.item")}`
        : `${childrenCount} ${i18n.t("options.config.sync.webdav.unresolved.items")}`
